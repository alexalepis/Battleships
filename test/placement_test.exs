defmodule PlacementTest do
  ...
  test "RANDOM placement of ship on 5x5 map (count occurences)" do
    ...
    assert Enum.reduce(new_board.map, 0, fn {_key, value}, acc ->
             if value == last_ship.id, do: acc + 1
           end) == 1
  end
  ...
  test "RANDOM placement of two ships on 5x5 map (count occurences)" do
    ...
    assert Enum.reduce_while(new_board.map, 0, fn {_key, value}, acc ->
             if value == first_ship.id, do: {:cont, acc + 1}, else: {:cont, acc}
           end) == 3

    assert Enum.reduce_while(new_board.map, 0, fn {_key, value}, acc ->
             if value == last_ship.id, do: {:cont, acc + 1}, else: {:cont, acc}
           end) == 1
  end
end
